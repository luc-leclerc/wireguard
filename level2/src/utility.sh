#!/bin/sh

# Used for either server or client
createKeyPair() {
  local privateKey=$(wg genkey);
  local publicKey=$(echo "$privateKey" | wg pubkey);
  echo "Public: $publicKey";
  echo "Private: $privateKey";
  echo "(For client only) Pre-shared key: $(wg genpsk)"
}

function convert_ipV4_to_hex() {
  printf "${1:-192.168.1.100}" | xargs -d '.' -n1 printf '%02X'
}

function convert_hex_to_decimal() {
  printf "ibase=16; %s\n" "${1:-FF}" | bc
}

function convert_ipV4_to_decimal() {
  convert_hex_to_decimal $(convert_ipV4_to_hex "${1:-192.168.10.100}");
}

# Ex.: `convert_decimal_to_ipV4 3232238180` -> 192.168.10.100
function convert_decimal_to_ipV4() {
  a=$((($1 & 4278190080) >> 24)) # echo $((255 << 24))
  b=$((($1 & 16711680) >> 16)) # echo $((255 << 16))
  c=$((($1 & 65280) >> 8)) # echo $((255 << 8))
  d=$(($1 & 255)) # the first 8 bit to 1 is 255
  printf "%s.%s.%s.%s\n" $a $b $c $d
}